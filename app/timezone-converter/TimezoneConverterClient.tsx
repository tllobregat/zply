...
import TimezoneCard from '@/app/timezone-converter/TimezoneCard';
...
export default function TimezoneConverterClient(): ReactNode {
  ...
  return (
    <ToolPageLayout
      ...
      breadcrumbItems={[
        { label: 'Time', href: `/?category=${Category.TIME}` },
      ...
      footerIndicator={
        <>
          <span className={cn('flex items-center gap-1.5 opacity-80', theme.text)}>
            <Zap className="w-3 h-3" /> {state.liveMode ? 'Real-time sync active' : 'Time editing mode'}
          </span>
          <Separator />
          <span>{state.selectedTimezoneNames.length} Timezones</span>
        </>
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <AnimatePresence mode="popLayout">
            ...
            >
              ...
              <span className="text-xs font-black uppercase tracking-widest text-muted-foreground group-hover:text-foreground">Add Timezone</span>
            </motion.button>
          </AnimatePresence>
        </div>
      </div>
