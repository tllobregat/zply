...
import { JsonToolbar } from '@/app/json-utils/JsonToolbar';
import { JsonTreeView } from '@/app/json-utils/JsonTreeView';
import { useJsonActions } from '@/app/json-utils/use-json-actions';
import { useJsonState } from '@/app/json-utils/use-json-state';
import { useJsonTransformation } from '@/app/json-utils/use-json-transformation';
            ...
            error
              ? (
                <span className="flex items-center gap-2 text-red-400">
                  <AlertCircle className="w-3.5 h-3.5" /> Invalid JSON: {error}
                </span>
              )
              : (
                <span className={cn("flex items-center gap-2 opacity-80", theme.text)}>
                  <Check className="w-3.5 h-3.5" /> JSON Valide
                </span>
              )
          }
          <Separator />
