      ...
      <div className="relative flex flex-col xl:flex-row items-center justify-between gap-8">
        <div className="space-y-3 max-w-lg">
          <span
            ...
            <Lock className="w-3.5 h-3.5 fill-current" /> {t(`${ToolId.MY_IP}.privacyGate`)}
          </span>
          <h2 className="text-2xl md:text-3xl font-black font-sans tracking-tight text-foreground">
            {t(`${ToolId.MY_IP}.externalQueryRequired`)}
          </h2>
          <p
            className="text-muted-foreground text-xs leading-relaxed"
            dangerouslySetInnerHTML={{ __html: t.raw(`${ToolId.MY_IP}.privacyGateDescription`) }}
          />
        </div>
        <div className="flex flex-col sm:flex-row xl:flex-col gap-3 shrink-0 w-full sm:w-auto">
          ...
          >
            ...
            {loading ? t(`${ToolId.MY_IP}.detecting`) : t(`${ToolId.MY_IP}.showMyIp`)}
          </Button>
          <Button
            variant="outline"
            disabled={loading}
