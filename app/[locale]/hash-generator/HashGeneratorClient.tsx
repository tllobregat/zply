...
import { HashInput } from '@/app/[locale]/hash-generator/HashInput';
import { HashResults } from '@/app/[locale]/hash-generator/HashResults';
import { useHashState } from '@/app/[locale]/hash-generator/use-hash-state';
import { Hashes, useHashTransformation } from '@/app/[locale]/hash-generator/use-hash-transformation';
...
export default function HashGeneratorClient(): ReactNode {
  ...
  return (
    <ToolPageLayout
      ...
      title={t(`${ToolId.HASH_GEN}.title`)}
      icon={<Hash className="w-5 h-5" />}
      breadcrumbItems={[
        { label: tCategories(Category.SECURITY), href: `/?category=${Category.SECURITY}` },
        { label: t(`${ToolId.HASH_GEN}.title`) }
      ]}
      headerActions={
        <ViewModeToggle viewMode={viewMode} setViewMode={setViewMode} />
      }
      footerIndicator={
        <span className="flex items-center gap-1.5"><Zap className="w-3 h-3 text-yellow-500/30" /> {useTranslations('Common')('toolIndicators.realTimeHashing')}</span>
      }
