...
import { EpochLiveNow } from '@/app/[locale]/epoch-converter/EpochLiveNow';
import { EpochResultCard } from '@/app/[locale]/epoch-converter/EpochResultCard';
import { EpochTimestampInput } from '@/app/[locale]/epoch-converter/EpochTimestampInput';
...
export default function EpochConverterClient(): ReactNode {
  ...
  return (
    <ToolPageLayout
      toolId={ToolId.EPOCH}
      title={t(`${ToolId.EPOCH}.title`)}
      icon={<Clock className="w-5 h-5" />}
      breadcrumbItems={[
        { label: tCategories(Category.TIME), href: `/?category=${Category.TIME}` },
        { label: t(`${ToolId.EPOCH}.title`) }
      ]}
      footerIndicator={
        <span className="flex items-center gap-1.5"><Zap className="w-3.5 h-3.5 text-yellow-500/30" /> {useTranslations('Common')('toolIndicators.msAutoDetected')}</span>
      }
      ...
      <div className="w-full max-w-2xl space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <EpochResultCard
            ...
            label={t(`${ToolId.EPOCH}.localTime`)}
            value={localDate}
