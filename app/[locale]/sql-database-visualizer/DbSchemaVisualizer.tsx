...
export function DbSchemaVisualizer(): ReactNode {
  ...
  return (
    <ToolPageLayout
      ...
      title={t(`${ToolId.DB_SCHEMA}.title`)}
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-2 text-green-500/60">
              ...
              <span className="hidden sm:inline">{tCommon('interactiveEngine')}</span>
            </span>
          </div>
          <Separator />
          <span className="text-muted-foreground truncate max-w-37.5 sm:max-w-none">
            {tCommon('tablesCount', { count: nodes.length })} • {tCommon('relationsCount', { count: edges.length })}
          </span>
        </div>
      }
    >
